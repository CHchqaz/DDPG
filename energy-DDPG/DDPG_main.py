...
import numpy as np
import time
from environment_DDPG import enva
...
class Critic(object):
    ...
    def learn(self, s, a, r, s_,i):
        ...
        result = self.sess.run(merged, feed_dict={S: s, self.a: a, R: r, S_: s_})
...
for i in range(MAX_EPISODES):
    ...
    for j in range(MAX_EP_STEPS):
        ...
        a = actor.choose_action(s/80)
        a = np.clip(np.random.normal(a, var), 0, 1)*40    # add randomness to action selection for exploration
        s_, r, done, info = env.step(s,a)
        #print('a.shape:',np.transpose(a).shape)
        print('a: ',a)
        print('r:',r)
        M.store_transition(s/100, (a/40)[0], r, s_/100)
        ...
        if M.pointer > MEMORY_CAPACITY:
            ...
            critic.learn(b_s, b_a, b_r, b_s_,k)
            actor.learn(b_s)
